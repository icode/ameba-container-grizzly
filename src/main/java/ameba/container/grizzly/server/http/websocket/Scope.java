 ...
 * @author icode
 */
public class Scope extends RequestScope {
    ...
    public Scope(RequestScope scope) {
        this.scope = scope;
        context = scope.createContext();
    }

    public static Scope wrap(RequestScope scope) {
        return new Scope(scope);
    }

    public static Scope create(InjectionManager manager) {
        return wrap(manager.getInstance(RequestScope.class));
    }
    ...
    public RequestContext activate() {
        try {
            RequestContext old = retrieveCurrent();
            activate(context, old);
            return old;
        } catch (Throwable throwable) {
            throw new WebSocketException(throwable);
        }
